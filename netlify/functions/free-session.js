...
<p style="font-size:.8rem;color:#888;font-family:sans-serif">Vous recevez cet email car vous avez activé les demandes d'écoutant dans votre profil.</p>`;
            ...
            results.filter(r => r.status === 'rejected').forEach(r => console.error('free-session alert email:', r.reason?.message || r.reason));
            console.log(`free-session: alerte Max envoyée à ${targets.length} destinataire(s)`);
