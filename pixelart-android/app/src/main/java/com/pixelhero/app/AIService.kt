...
object AIService {
    ...
    /**
     * Edit / re-pose a reference image via OpenAI gpt-image-1 `/v1/images/edits`.
     * Sends the bitmap + a prompt; the AI redraws the SAME character following
     * the prompt instructions. Used for animation frames and view rotations
     * so the output character stays consistent with the user's existing sprite.
     *
     * @param refBitmap the sprite to use as visual reference (will be PNG-encoded)
     * @param prompt instruction text (e.g. "same character, sword raised overhead, side view")
    ...
    fun editOpenAI(refBitmap: Bitmap, prompt: String, apiKey: String, size: String = "1024x1024", quality: String = "medium"): Bitmap? {
        ...
        val refPng = ByteArrayOutputStream().use { os ->
            refBitmap.compress(Bitmap.CompressFormat.PNG, 100, os)
            os.toByteArray()
        }
        repeat(2) { attempt ->
    ...
    private fun editOpenAIOnce(refPng: ByteArray, prompt: String, apiKey: String, size: String, quality: String): Bitmap? {
        return runCatching {
            ...
            val safePrompt = prompt.replace("\n", " ").replace("\r", " ").replace("\t", " ").take(3900)
            conn.outputStream.use { out ->
                writeMultipartFile(out, boundary, "image", refPng, "sprite.png", "image/png")
                writeMultipartText(out, boundary, "prompt", safePrompt)
            ...
            if (code != 200) {
                val errBody = (conn.errorStream ?: conn.inputStream).bufferedReader().use { it.readText() }
                lastError = "edits HTTP $code: ${errBody.take(400)}"
                conn.disconnect()
                return@runCatching null
            }
            val response = conn.inputStream.bufferedReader().use { it.readText() }
            val b64 = extractB64Json(response)
                ?: run { lastError = "edits: réponse sans b64_json: ${response.take(200)}"; return@runCatching null }
            val bytes = android.util.Base64.decode(b64, android.util.Base64.DEFAULT)
            BitmapFactory.decodeByteArray(bytes, 0, bytes.size)
                ?: run { lastError = "edits: image décodée invalide"; null }
        }.onFailure { lastError = "edits: ${it.message ?: it.javaClass.simpleName}" }.getOrNull()
    }

    private fun writeMultipartFile(out: OutputStream, boundary: String, name: String, data: ByteArray, filename: String, contentType: String) {
        out.write("--$boundary\r\n".toByteArray())
